                    ...
                    <thead class="bg-gradient-to-r text-black">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">
                                Status
                                <div class="text-xs font-normal text-gray-400 mt-1">Payment</div>
                            </th>
                                        ...
                                        {{-- @if ($product->status == 1)
                                            ...
                                            <div id="deleteProductModal-{{ $product->id }}" tabindex="-1"
                                                ...
                                                <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
                                                    <button type="button"
                                                        class="absolute top-2 right-2 text-gray-400 hover:text-gray-600"
                                                        data-modal-hide="deleteProductModal-{{ $product->id }}">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                                    ...
                                                    <h3 class="text-lg font-semibold text-gray-900 mb-2 text-center">Delete
                                                        Product</h3>
                                                    ...
                                                    <div class="flex justify-end space-x-3">
                                                        <button type="button"
                                                            class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500 transition-colors duration-200"
                                                            data-modal-hide="deleteProductModal-{{ $product->id }}">
                                                            Cancel
                                                        </button>
                                                        <form
                                            ...
                                            <div id="deleteProductModal-{{ $product->id }}"
                                                ...
                                                <div class="bg-white rounded-2xl p-8 max-w-md w-full mx-4 transform scale-95 transition-transform duration-300"
                                                    ...
                                                    <div class="text-center">
                                                        ...
                                                        <p class="text-gray-600 mb-8">Are you sure you want to delete this product? This action <br>cannot be undone and will permanently remove the <br>product from the system.</p>
...
@section('custom_js')
    <script>
        $(document).ready(function() {
            ...
            $('form[method="POST"]').on('submit', function(e) {
                if ($(this).find('button[type="submit"]').text().includes('Delete')) {
                    if (!confirm(
    ...
    <script>
        ...
        @if ($latestOrder)
            document.querySelectorAll('[data-modal-toggle="orderStatusModal-{{ $latestOrder->id }}"]').forEach(btn => {
                btn.addEventListener('click', function() {
                    document.getElementById('orderStatusModal-{{ $latestOrder->id }}').classList.remove(
                        'hidden');
                });
            });
            document.querySelectorAll('[data-modal-hide="orderStatusModal-{{ $latestOrder->id }}"]').forEach(btn => {
                btn.addEventListener('click', function() {
                    document.getElementById('orderStatusModal-{{ $latestOrder->id }}').classList.add(
                        'hidden');
                });
            });
        @endif
        ...
        document.addEventListener('click', function(event) {
            const modals = document.querySelectorAll('[id^="orderStatusModal-"], [id^="deleteProductModal-"]');
